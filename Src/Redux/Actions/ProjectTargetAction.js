...
export function getProjectTarget() {
    return async (dispatch) => {
        ...
        dispatch(projectTargetDispatch({ isLoading: true }, GETPROJECTTARGET_PROGRESS))
        try {
            const res = await request({ url: '/project-target', method: 'GET' });
            console.log("Project Target Response", res.data.data)
            dispatch(projectTargetDispatch(res.data.data.project, GETPROJECTTARGET_SUCCESS))
        } catch (error) {
...
export function updateProjectTarget(resource, date, id, navigation) {
    return async (dispatch) => {
        dispatch(projectTargetDispatch({ isLoading: true }, EDITPROJECTTARGET_PROGRESS))
        var formData = {
            "resource": resource,
            "date": date
        }
...
export function getResources() {
    return async (dispatch) => {
        ...
        try {
            ...
            dispatch(projectTargetDispatch(data.data.data.resources, GETRESOURCE_SUCCESS))
        } catch (error) {
